...
export default async function AdminBanderPage() {
  const session = await getServerSession(authOptions);
  const u = session?.user;
  if (!u?.discordId) {
    redirect("/api/auth/signin/discord?callbackUrl=/admin/bander");
  }
      ...
      <div className="mx-auto max-w-[1100px] px-4 py-8 sm:px-6 lg:py-10">
        <div className="mb-6">
          ...
          <p className="mt-1.5 text-[14px] text-foreground-muted">
            Society-økonomi, medlemmer, bosser og hvornår de sidst var online.
            Klik en bande for at se medlemmerne.
          </p>
        </div>
        <CompaniesOverview
          initial={gangs}
          variant="gang"
          apiBase="/api/admin/gangs"
        />
      </div>
    </main>
  );
